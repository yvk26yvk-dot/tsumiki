...
### メールテンプレート(日英併記) — 今回はスキップ(2026-07-10決定)

**現行のSupabase無料構成ではテンプレート編集に独自SMTP設定またはProアップグレードが
必要と判明したため、今回は差し替えず、デフォルトの英語メールのまま進める。**
日英併記化(文面案は実装計画Task 3 Step 2に保存済み)は、App Store公開前に
独自SMTP(Resend等)を導入するタイミングでまとめて実施する。

- 方針(実施時): 本文上段に日本語(コウテイちゃんのトーン・責めない)、下段に英語。
  リンクは `{{ .ConfirmationURL }}`。ユーザーごとの言語出し分けは標準機能にないため
  併記が現実解。言語別配信は第三段階でSend Email Hookとともに検討。
...
※旧項目6(日英併記の文面確認)は、テンプレート差し替えのスキップに伴い削除。
  メールはデフォルトの英語文面のまま(SMTP導入時に再検証)。

## スコープ外

- Capacitor iOS化(次スペック。XcodeのDL完了後)
- 独自SMTP(Resend等)+認証メールの日英併記・言語別配信(App Store公開前に別途。
  無料構成ではテンプレート編集不可のためSMTP導入とセットで実施)
- メールアドレス変更機能
